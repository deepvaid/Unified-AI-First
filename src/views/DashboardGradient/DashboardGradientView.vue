...
// Gradient exploration — a COPY of the Overview dashboard (DashboardView.vue)
// pinned to the `socialGradient` palette, which today is an exact clone of
...
import { applyChartPalette, CHART_THEMES, type ChartPalette } from '@/plugins/chartPalette'
...
onErrorCaptured((err) => {
  ...
  console.error('[DashboardGradientView Error]', err)
  renderError.value = message
  return false
})

// This page is the gradient showroom: ?chart= picks WHICH gradient variant to
// wear (socialGradient by default); non-gradient ids fall back to the default.
// On the way out, hand control back to App.vue's ?chart= handling so leaving
// here never leaves the production dashboard wearing an exploration palette.
const GRADIENT_PALETTES: readonly ChartPalette[] = [
  'socialGradient', 'grayBlueGradient', 'grayBlueGoldGradient', 'oceanGradient',
]
const isGradientPalette = (v: unknown): v is ChartPalette =>
  typeof v === 'string' && (GRADIENT_PALETTES as readonly string[]).includes(v)
const pinGradientPalette = () => {
  applyChartPalette(isGradientPalette(route.query.chart) ? route.query.chart : 'socialGradient')
}
onMounted(pinGradientPalette)
watch(() => route.query.chart, pinGradientPalette)
onBeforeUnmount(() => {
  const q = route.query.chart
  applyChartPalette(typeof q === 'string' && q in CHART_THEMES ? (q as ChartPalette) : 'grayBlue')
})
...
// Switching dashboards stays inside the exploration copy — the production
// Dashboard/DashboardDetail routes are never pushed from here.
function dashboardRouteLocation(dashboard: Dashboard) {
  if (dashboard.isDefault) {
    return { name: 'DashboardGradient' as const, params: { accountId: dashboard.accountId } }
  }
  return { name: 'DashboardGradientDetail' as const, params: { accountId: dashboard.accountId, dashboardId: dashboard.id } }
}

function navigateToDashboard(dashboard: Dashboard | undefined) {
  if (!dashboard) {
    router.push({ name: 'DashboardGradient', params: { accountId: accountId.value } })
    return
  }
...
watch(
  ...
  () => {
    if (!activeDashboard.value) {
      router.replace({ name: 'DashboardGradient', params: { accountId: accountId.value } })
    }
  },
  { immediate: true },
)
