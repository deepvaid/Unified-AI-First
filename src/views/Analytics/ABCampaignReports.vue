...
const filterWinnerQuickFilter = {
  key: 'winner',
  label: 'Winning Variant',
  options: (['Variant A', 'Variant B']).map((v) => ({ label: v, value: v })),
}
...
<template>
  <div class="h-100 d-flex flex-column gap-5">
    ...
    <v-card variant="flat" border rounded="lg" class="flex-grow-1 d-flex flex-column overflow-hidden">
      <MpDataTableToolbar
        v-model:quick-filter-value="filterWinner"
        :quick-filter="filterWinnerQuickFilter"
