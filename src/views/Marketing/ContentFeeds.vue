...
const headers = [
  { title: 'Feed Name', key: 'name', sortable: true },
...
const feeds = ref([
  { name: 'Latest Blog Posts (RSS)', url: 'https://blog.example.com/rss', status: 'Active' },
  { name: 'Daily Deals JSON', url: 'https://api.example.com/deals', status: 'Active' },
  { name: 'Legacy XML Feed', url: 'https://example.com/feed.xml', status: 'Error' },
])
...
<template>
  <div class="h-100 d-flex flex-column gap-5">
    ...
    <v-card variant="flat" border rounded="lg" class="flex-grow-1 d-flex flex-column overflow-hidden">
      ...
      <v-data-table :headers="headers" :items="feeds" :search="search" hover density="comfortable" :items-per-page="15" fixed-header class="flex-grow-1">
        ...
        <template v-slot:item.actions>
          <v-menu>
            ...
            <v-list density="compact" rounded="xl" nav min-width="160" elevation="8">
              ...
              <v-list-item prepend-icon="refresh-cw">Sync Now</v-list-item>
