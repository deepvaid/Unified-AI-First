...
## A12 flow specs (action-flow crawl, 2026-07-11)

<!-- Field-level create/edit flow detail from 3 delegated haiku browser crawls; drove the flow-parity build (commits d10c0ee, f974336, c3dc709). -->

- **Search Pinning create/edit** (`/solutions/search/pinning/create/:query`): full-page, NOT a drawer. Query field + products-per-row (4) / per-page (24) dropdowns + search-within-results; ranked results grid, each card has a checkbox + pin icon; "DROP PRODUCTS HERE TO PIN" zone; Save top-right. → BUILT as SearchPinningEditor two-pane workspace (query→results, pin/drag-rank, bulk, dirty Save). Per-row/page dropdowns dropped (presentational in a mock).
- **Merchandising Rules create** (3-step wizard): Name (required) → Search terms (add-to-list, skippable) → Conditions. Condition type = **Promote / Pin / Only include / Exclude**; "Apply to" = Both/Product/Variant; Promote shows a **boost slider −99…90** with Bury/Lower/Higher/Boost quick presets; max 9 Promote conditions. → BUILT into SearchRules drawer condition builder (action 'pin' + applyTo + slider/presets + 9-max warning). Kept as a drawer (not a stepper) — same fields.
- **Promo Cards create** (3-step): Title → Search terms → manage cards. **Banners create** (multi-step): Title (validated despite "optional") → image → search terms. → promo/banner create drawers gained the terms combobox.
- **Collections edit** (3 tabs, not a wizard): Shopify Filters (use-Shopify checkbox) / Activation (page URL) / Configured Filters & Sorting (filter rows: include·field·equals·value, All-conditions logic, Sort by dropdown; per-row edit pencils). → BUILT as CollectionEditor.vue (closes B-A12 #11).
- **Recommendations create** (new dashboard): step 1 = page-type card selector (Home/Product/Collection/Cart/Custom). Existing EngineEditor already covers page+type+counts — left as-is.
- **Synonyms** (new dashboard `maropost-dashboard.findify.io/search/synonyms`): ADD NEW = inline row (status toggle · Type one-way/two-way · Queries input · Leads-to input · ✓/✗ confirm). UPLOAD = "Bulk Synonyms Upload" dialog: Synonym Type dropdown + CSV file (2 cols: terms, synonyms; rows = groups) + Download sample CSV. → NOT yet built in prototype (Synonyms list exists; add-new + upload affordances pending).
- **Smart Collections Pinning/Rules**: same tools as Search versions, scoped to a collection (breadcrumb "Default Merchandising", tabs Pinning / Merchandising Rules / Promo Cards[Coming Soon]); rules list = Active toggle + Rule Title. Prototype's Default Merchandising view already covers this.
- **Cross-flow**: legacy Save disabled during wizard steps; inline field-level validation; **no delete confirmation dialogs anywhere** (our builds add MpConfirmDialog — deliberate UX upgrade).
- **Coverage gap**: Settings ▸ Primary/Advanced Setup subpages never captured — the OLD dashboard (dashboard.findify.io) hangs on load across 3 attempts; the NEW dashboard has no Settings equivalent. B-A12 #22 stays partial pending a future crawl when the old dashboard is responsive.
...
| 4 | Acquisition Forms | AcquisitionForms | exists | rebuilt | d22e7cc | Rebuilt to UAT (116000): folders, search, script-link dialog, Drag and Drop/Legacy type. Invented KPI/metrics/grid removed |
| 5 | Landing Pages | LandingPages | exists | rebuilt | 3a677ee | Rebuilt to UAT (116000): 3-value editor type, folders, search, Verify Domain / Preview / Duplicate row menu |
...
| 10 | Email Content | EmailContent | exists | rebuilt | eb12f75 | Rebuilt to UAT (116000): 489-style table, folders, search, archives, 6-item row menu, Used-by column |
...
| 29 | Acquisition Forms — template picker | FormSelection | exists | rebuilt | d22e7cc | Now a full route /acquisition/forms/select as in UAT (was an in-list dialog); 7 cards w/ drawn previews |
| 30 | Acquisition Forms — 5-step builder | FormBuilder | exists | rebuilt | d22e7cc | Reordered to UAT's Details/Settings/Design/Content/Finished; labelled, click-to-jump stepper kept |
| 31 | Landing Pages — template library | LandingPageTemplates | missing→built | done | 6438e8e, 3a677ee | Path aligned to UAT's /landing_pages/template (plural redirects); Library/My-templates tabs, 4 facet groups, routable tab |
...
| 34 | Lead Ads (social_leads) | LeadAds | missing→built | done | 35f9af7 | Route previously reused AcquisitionForms.vue — a different feature. New list + useSocialLeads store |
| 35 | Lead Ads — create / edit | LeadAdForm | missing→built | done | 35f9af7 | Single-page four-section form, as in UAT; serves /new and /:id/edit |
| 36 | Email Content — Select Template | ContentTemplates | missing→built | done | eb12f75 | /contents/template; Library + My-templates tabs, 4 facet groups, routable tab |
| 37 | Email Content — editor chooser (4 options) | ContentEditorChooser | missing→built | done | eb12f75 | /contents/select; one component serves both choosers, told apart by route meta |
| 38 | Email Template — editor chooser (2 options) | ContentEditorChooser | missing→built | done | eb12f75 | /content_templates/select_editor |
| 39 | Email Template — layout step | TemplateLayouts | missing→built | done | eb12f75 | /content_templates/layouts; 6 layouts, Drag & Drop path only |
| 40 | Email Content — preview | EmailContentPreview | missing→built | done | eb12f75 | /contents/:id/preview; desktop/mobile toggle, edit + campaign handoffs |
| 41 | Archives | ContentArchives | missing→built | done | eb12f75 | /archive; re-parented under Email Content (UAT breadcrumbs it under Settings with no way back) |
| 42 | Landing Pages — Select Builder | LandingPageBuilderChooser | missing→built | done | 3a677ee | /landing_pages/create; UAT's tiles are PNGs absent from the a11y tree |
...
| 2 | Search ▸ Pinning | MerchandisingChannelSearchPinning + PinEdit | missing→built | done | 1fc7ae5, d10c0ee | Full-page pin editor (query→results, pin/drag-rank, bulk) — flow parity |
| 3 | Search ▸ Merchandising Rules | MerchandisingChannelSearchRules | partial→built | done | 1fc7ae5, f974336 | Condition builder: Promote/Pin/Only-include/Exclude + Apply-to + boost slider — flow parity |
| 4 | Search ▸ Promo Cards | MerchandisingChannelSearchPromos | missing→built | done | d179693 | Card gallery + status toggles |
| 5 | Search ▸ Banners | MerchandisingChannelSearchBanners | missing→built | done | d179693 | Card gallery + target URL |
| 6 | Search ▸ Blacklisting (suggestions + product matches) | MerchandisingChannelBlacklisting | missing→built | done | 1fc7ae5 | Two tabs |
...
| 10 | Search ▸ Content (BETA) | MerchandisingChannelContent | missing→built | done | 1fc7ae5 | Light list per BETA scope |
| 11 | Smart Collections ▸ Collections (+ edit tabs) | MerchandisingChannelCollectionEdit | partial→built | done | c3dc709 | CollectionEditor: Shopify Filters / Activation / Filters & Sorting tabs |
...
| 13 | Smart Collections ▸ Promo Cards / Banners | MerchandisingChannelCollectionPromos/Banners | missing→built | done | d179693 | Reuse Search views w/ collection scope |
| 14 | Recommendations | MerchandisingChannelRecommendations | exists | | | Engine types align w/ crawl (7 types) |
| 15 | Analytics ▸ Snapshot | MerchandisingChannelAnalytics | missing→built | done | 207af47 | KPIs + revenue chart |
| 16 | Analytics ▸ Search | MerchandisingChannelAnalyticsSearch | missing→built | done | 207af47 | Top searches / zero results tables |
| 17 | Analytics ▸ Smart Collections | MerchandisingChannelAnalyticsCollections | missing→built | done | 207af47 | Top collections table |
| 18 | Analytics ▸ Recommendations | MerchandisingChannelAnalyticsRecommendations | missing→built | done | 207af47 | Widget performance table |
| 19 | Settings ▸ Status & usage | MerchandisingChannelSetup | partial→built | done | 207af47 | Setup page exists; needs status cards + [REDACTED] snippet/key blocks |
| 20 | Settings ▸ Product Sync | MerchandisingChannelProductSync | missing→built | done | 207af47 | Sync overview + indexed counts |
| 21 | Settings ▸ Integrations | MerchandisingChannelIntegrations | missing→built | done | 207af47 | Rating & Reviews cards |
...
- 2026-07-11 — **CDP + Analytics + Products ACTION-FLOW parity pass** (user-directed; UAT account 116000, multi-agent crawl + build). Crawled the legacy action flows across all three modules (3 parallel haiku crawlers + a self-run Products deep-crawl after the delegate hit a domain gate), then rebuilt the prototype to parity — dead controls wired to real store-persisted flows, no new features. Commits:
  ...
  - **Products base** 5a37e2c + **completion** c192321: base pass (rules builder, inventory adjust/transfer w/ MpKpiCard, reservations, tax categories, CSV export util). Completion after deep-crawl of the REAL flows: full-page 3-step New Product wizard (Details/Organise/Variants + per-location variant stock), 3-step New Kit wizard (component picker), CSV+FTP 4-step import wizards, scoped Export dialog (Current Page/All/Selected/search + filename), automated-Collections list; header NEW PRODUCT + IMPORT dropdowns; new ProductWizard/KitWizard/ProductImportWizard views + 5 fullPage routes.
  - Shared: new src/utils/exportCsv.ts (downloadCsv) powers every Export. Type-check + production build clean; each module verified in preview (0 console errors). PARITY NOTES for a later decision: legacy /product_recommendations is really a Product Catalog + Feeds manager (kept our rules concept per no-new-features); legacy web-tracking is gated on account 116000 (uncrawlable); MpStatusChip has no icon for type='general' (show-icon is a no-op there). Analytics + Products-base pushed earlier (c76e7b2); CDP (ad763d4) + Products-completion (c192321) committed locally.
...
- 2026-08-30 — **Marketing Acquisition + Content slice BUILT** (`docs/rebuild/marketing-acquisition/`). Crawled all four groups from account **116000** (Acquisition Forms, Landing Pages, Lead Ads, Email Content); the brief's 10 URLs expanded to ~20 surfaces once header actions were followed one level deep. User-locked decisions: build everything found · remove the July redesign's invented KPI/metrics/grid UI (UAT has no metrics in this module) · folders via `MpFolderSelect` + `?folder=` rather than a new tree component · add search to every list. Four commits, one per group: Lead Ads (35f9af7 — the route previously reused AcquisitionForms.vue, a different feature entirely), Acquisition Forms (d22e7cc), Email Content (eb12f75 — 6 new surfaces), Landing Pages (3a677ee). Cross-cutting fixes applied everywhere: search, URL-backed filter state, real empty/loading/error states, a floating bulk bar that no longer unmounts the page's primary CTA, keyboard-reachable row menus with destructive items last behind a divider, and accessible names on every checkbox/switch/icon button. Two shared-component fixes fell out: `MpFormField` set `aria-required` on `role="group"` (ARIA disallows it — a WCAG 4.1.2 failure inherited by every consumer), and `MpStatusChip` gained verified/unverified tones. `useFolders` gained the `forms`/`landing_pages`/`content_templates` scopes plus UAT's owner-vs-shared privacy model. Verified: `npm run build` passes, axe-core WCAG 2.1 AA **0 violations** across all 14 rebuilt surfaces, no horizontal overflow at 375px, every route resolves and renders. Four component gaps logged (`marketing-acquisition/GAPS.md`). Everything mutating on UAT stayed unexecuted, so all success/failure states remain inferred.
...
- 2026-07-11 — **A12 Merchandise ACTION-FLOW parity** (user: "deep crawl of all action flows … still some items left", pointed at the pinning create URL). 3 delegated haiku flow-crawls captured field-level create/edit specs (digest: "A12 flow specs" section above). Built: (1) d10c0ee full-page **SearchPinningEditor** replacing the pin drawer — query-driven two-pane workspace w/ rank badges + drag-reorder + bulk pin, matching Findify's create/:query flow; (2) f974336 **rule condition builder** upgraded to Findify's model (Promote/Pin/Only-include/Exclude + Apply-to Both/Product/Variant + boost slider −99…90 w/ Bury/Lower/Higher/Boost presets + 9-promote-max) and promo/banner create drawers gained the search-terms trigger field; (3) c3dc709 **CollectionEditor** (3 tabs: Shopify Filters / Activation / Filters & Sorting) closing B-A12 #11. All verified in preview (pin editor pin→save→list count, collection tabs + save, rule condition builder), type-check clean per commit, 0 console errors. **Still pending** (recorded, not built): Synonyms ADD-NEW inline row + Bulk CSV upload affordances; Settings ▸ Primary/Advanced Setup subpages (B-A12 #22) — the legacy dashboard that hosts them hangs on load, so uncrawlable this pass. NOT pushed (awaiting user).
- 2026-07-10 — **A12 MERCHANDISE CLOUD: full Findify parity BUILT** (user directive: "full Findify parity now", multi-agent expedited, cheap models for crawl/audit). Six commits e343a69→1d8454b: (1) e343a69 foundation fixes — nav traps (6 views' account-level links → channel-scoped named routes; verified: pinning editor opens in-shell where it previously bounced to the selector), AppSidebar Merchandise collapsed to single entry, layout context-strip removed, route-based switcher, style fixes; (2) 6c3aca5 useMerchandising channel-scoped (delegated agent; exposed list names unchanged; Atlas vs Fashion Boutique differentiated seeds; activeStore mirrors the active channel so subtitles name the right storefront) + new entities (SearchPin/PromoCard/MerchBanner/Blacklist/SyncInfo/module analytics); (3–5) 12 new pages via 3 parallel build agents — Search Pinning/Rules/Blacklisting/Content (1fc7ae5, + additive SearchRule entity), scope-shared Promo Cards/Banners (d179693), Analytics ×4 via meta.reportKind + Product Sync + Integrations + Status&usage upgrade w/ [REDACTED] key (207af47, + v-row flex-grow-0 KPI stretch fix); (6) 1d8454b IA wiring — 14 routes, menu rebuilt to crawled Findify IA w/ unique match sets (multi-highlight collisions gone), MerchandisingCapabilityPlaceholder deleted. Remaining B-A12 partials: #11 Collections edit tabs, #22 Primary/Advanced setup extras — low priority, recorded. Verified end-to-end at 1280: every rail item → real distinct page, channel switcher swaps data, 0 new console errors, type-check clean per commit.
