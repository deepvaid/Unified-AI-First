...
**Summary: 112 rows → 79 exists · 13 partial · 20 missing.** Build queue (all partial + missing rows) = 33; **6 built in A05 Marketing + 2 in A06 Commerce (2026-07-08)**, 25 remaining. Rows with `done` build status = 15 (7 earlier Marketing back-fills + 6 Marketing slice + 2 gift-cards slice).
...
| 25 | Email Campaign — report (Dashboard/Overlay/ISP/Details tabs) | CampaignReport | missing→built | done | aea9f25 | New /campaigns/:id/report; Dashboard/Overlay/ISP/Details tabs from campaigns store. Entry-wiring from list deferred (additive contract) |
| 26 | Transactional Email — create form | CreateTransactional | missing→built | done | 071cd46 | New /transactional_campaigns/new full-page form. Entry from list deferred (additive contract) |
| 27 | Campaign Tag — create modal | CreateCampaignTag | partial→built | done | 071cd46 | New /campaign_tags/new full-page create (colour picker + preview). Built as page vs in-list modal (additive contract). Entry deferred |
| 28 | Survey — create modal | CreateSurvey | partial→built | done | 071cd46 | New /surveys/new full-page create (close date/quota/branding). Built as page vs in-list modal. Entry deferred |
...
| 31 | Landing Pages — template library | LandingPageTemplates | missing→built | done | 6438e8e | New /landing_pages/templates; category filters + Library/My-Templates tabs + nav item |
| 32 | Email Content — drag & drop editor | EmailContentEditor | missing→built | done | 9422b99 | New /contents/editor/:id; palette + live canvas + settings panel. Entry from list deferred (additive contract) |
