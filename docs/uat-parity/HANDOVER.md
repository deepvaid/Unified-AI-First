...
- **Phase 2 (Part B gap matrix) — DONE (2026-07-07).** All 112 Part A rows matched to `src/router/index.ts` by feature intent and adversarially verified: **79 exists / 13 partial / 20 missing**; 9 verdicts corrected by the adversarial pass (⚑ in the tracker); built Marketing rows back-filled as `done`. A07 (1) + A11 (35) verdicts are PROVISIONAL — matched from titles only, re-confirm after the logged-in crawl. Build queue = 33 pending partial/missing rows.
- **Phase 3 (prioritize) — DONE (2026-07-07).** Build order approved: A05 Marketing → A06 Gift Cards → A02 Analytics → A08 Chatbots → A03 Web Tracking; A11 Settings deferred until the logged-in crawl. See tracker "Phase 3 — Proposed build order".
- **Phase 4 (build) — IN PROGRESS. A05 Marketing DONE (2026-07-08). ← next slice: A06 Gift Cards.** A05 slice built 6 additive pages (Campaign Report #25, Landing Page Templates #31, Transactional/Survey/Campaign-Tag creates #26/#27/#28, Email Content editor #32) — commits in the tracker Progress log + Part B `done` rows. Build queue now 27 remaining. Earlier out-of-sequence builds: Marketing journeys, data journeys, email-campaign create (JourneyBuilder.vue + Journeys CTA were intentionally redesigned per explicit user instruction — an exception to additive-only, not a precedent). KNOWN LIMITATION carried forward: new create/detail/editor pages are route-reachable but their entry buttons on existing list views are inert — wiring them is an existing-template edit, deferred pending an explicit additive-exception. Build-phase style directive: clean/modern, flat cards + Material-style form fields, existing Vuetify + `Mp*` + tokens (no new UI dependency).
