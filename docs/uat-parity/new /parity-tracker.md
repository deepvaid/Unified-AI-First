...
| # | UAT page/flow | Prototype route | Verdict | Build | Commit | Notes |
|---|---|---|---|---|---|---|
| 1 | Online sales-channel selector | — | missing | pending | | Global Merchandise entry opens the account-level overview; no selector backed by `useSalesChannels` |
| 2 | Channel-scoped workspace + Merchandise section rail | — | missing | pending | | `MpSectionRail` and Store Editor shell exist, but Merchandising routes are standalone account-scoped routes |
| 3 | Channel overview | MerchandisingHome | partial | pending | | KPI/activity UI exists, but its duplicate store picker only changes labels; data remains global |
| 4 | Merchandise connection and sync health | SalesChannelDetail | partial | pending | | Canonical channels expose connection/status/health and a Merchandise handoff, but no capability-level state, diagnostics, or recovery action |
| 5 | Invalid, removed, offline, or cross-account recovery | — | missing | pending | | No Merchandise route ownership check or online-channel recovery state |
...
| 21 | Channel setup / integrations / product sync | SalesChannelDetail | partial | pending | | Existing channel status and setup entry are reusable; Merchandise handoff query is ignored and setup actions are toast-only |
| 22 | Canonical provider and Merchandise connection model | — | missing | pending | | `MerchStore` duplicates SalesChannel identity; SalesChannel lacks typed provider and capability-level sync diagnostics |
...
| 24 | Same-section channel switching + editor fallback | MerchandisingHome duplicate picker | partial | pending | | A store selector changes visible metadata, but not route context or datasets; Store Editor has a reusable section-root switch pattern |
...
|---|------|---------------------|---------|-----------------------------|---------------------|-----------------------------|-------------------|--------------|----------------------------|----------|------------|---------------|
| 1 | Entry | Global Merchandise navigation | Open module | — | — | `MerchandisingHome` | missing | Selector before workspace | Current entry silently picks a duplicate store context; no provider/status/health scan | critical | Replace home entry with an online-channel selector sourced from `useSalesChannels` | verified-code |
| 2 | Entry | Sales Channel detail handoff | Manage Merchandise | — | — | `SalesChannelDetail.openConnectedProduct` | partial | Direct to channel overview | Query handoff is discarded by the destination, so visible context can disagree with source | critical | Route directly to canonical `accountId/channelId` Merchandise overview | verified-code |
| 3 | Workspace | Channel identity and section navigation | Enter connected channel | — | — | `MpSectionRail` / no Merchandising layout | missing | Persistent Merchandise rail | Existing global submenu consumes space and does not establish local ownership | high | Add `MerchandisingLayout`, `MerchandisingSidebar`, and data-driven menu using `MpSectionRail` | verified-code |
| 4 | Switching | Switch channel from list/overview | Choose rail identity option | — | — | Duplicate picker in `MerchandisingHome` | missing | Rail identity switcher | Picker changes only store metadata; same data remains visible, creating false isolation | critical | Switch named routes with `accountId + channelId`, preserving supported list sections | verified-code |
...
| 7 | Recovery | Invalid, removed, offline, or cross-account channel | Open deep link | — | — | `StoreEditorLayout` pattern | partial | Workspace recovery state | Store Editor handles missing IDs, but offline children can still render and Merchandising is not nested there | high | Add online-only route validation and return-to-selector action | verified-code |
| 8 | Health | Disconnected or setup-required channel | Select channel | — | — | Sales Channel status only | missing | Channel Setup | No Merchandise capability setup state; current setup CTA is toast-only | high | Add channel-scoped connection/setup page and capability-derived status | verified-code |
| 9 | Health | Sync issue | Open affected channel | — | — | Sales Channel health rollup only | missing | Overview health banner | No per-capability reason, last sync, or recovery action | high | Add Merchandise sync diagnostics and prominent recovery banner | verified-code |
| 10 | Overview | Connected channel KPIs and activity | Land on overview | — | — | `MerchandisingHome` | partial | Overview | Information hierarchy is useful, but values are global and quick-create labels often open list pages | high | Scope KPIs/activity and route quick actions to actual create flows | verified-code |
...
| 25 | Setup | Provider integration and product sync | Connect or recover channel | — | — | `useSalesChannels` + toast-only setup | missing | Channel Setup | Provider is not canonical; connection state is duplicated and sync health lacks diagnostics | critical | Add typed provider/capability connection model and derive status/health from it | verified-code |
...
- Code verdicts: 0 fully channel-ready, 14 partial, 13 missing; authenticated live verification remains blocked for all reference-app states.
