...
| 5 | Analytics/ERFMReport.vue | /accounts/2000290/erfm_report | standard | done | | **Page replaced 2026-08-31** (UAT deep-crawl rebuild — the audited card-grid page was an invented design, not this URL). Re-audited after the rebuild: 0 horizontal overflow at 375px, matrices scroll in their own containers, MpTableSkeleton loading in all 4 data sections, MpEmptyState present, zero px literals / hardcoded colours, contrast 244/244 pass. The earlier note about no card-grid skeleton convention no longer applies — the sections are row×column shaped, so MpTableSkeleton fits. Docs: `docs/rebuild/erfm-report/` |
