...
| 8 | Commerce/CustomGiftCards.vue | /commerce/2000290/custom_gift_cards | standard | done | c23c000 | 15 audit findings applied: Export→outlined + drawer CTA→flat; artwork tiles opaque so --accent-on holds (was ~2.5:1, now 5.20/17.73:1); tile geometry/hover/focus/selected on tokens; 2 text-h4 → .mp-kpi-value; eyebrow → .mp-meta-label; 0.6rem!important caption → fontSize-11; icons 22→18; inline 96px → space token; aria-pressed on all 6 artwork tiles; 8 MpListRow → dl.mp-label-value. Plus 2 untiered columns tiered (375 was 6 cols/268px overflow → 3 cols/87px) |
| 9 | Commerce/PurchasableGiftCards.vue | /commerce/2000290/purchasable_gift_cards | standard | done | 4911c38 | 11 audit findings applied: Export→outlined + drawer CTA→flat; raw v-btn-toggle → MpSegmentedControl (labels kept, aria-pressed verified); filter drawer sheds pa-4 + ad-hoc heading; remove-denomination icon 21→18 with tooltip; placeholder icon 28→20; placeholder pa-6 + dashed !important + lying cursor:pointer → card token inset, no !important; rgba hairline → --border-subtle; eyebrow → .mp-meta-label; mono → token; validation message role=alert |
...
| 3 | Service/ChatbotList.vue | /accounts/2000290/chatbot | standard | done | f74014e | responsive column tiers (was 7 untiered, kebab off-screen at 375); raw v-chip + statusColor map → MpStatusChip sm; py-10 off plan-gate empty state; search-aware empty state with Clear search; MpFilterTabs controls-id + card id (tabs rendered aria-controls="undefined"); MpTableSkeleton + useInitialLoad |
...
| 13 | Products/PriceLists.vue | /commerce/2000290/retail/pricing | standard | done | 68874e3 | responsive column tiers; toolbar column menu was INERT (hidden-columns never bound) → merged into the composable, verified working; py-10 off empty state; .pl-desc 280px → 28ch; MpTableSkeleton + useInitialLoad; asymmetric decorative green off the Adjustment cell. Residual: table still scrolls ~89px inside its card at 375 (Title minWidth literal) — consistent with siblings, see progress log |
...
- 2026-09-05 — **Verification pass over the whole aesthetic pass** (15 module auditors over the 145 changed files, each candidate finding put to independent skeptics; 64 candidates, 3 upheld, 24 refuted). Regressions the pass introduced, now fixed in 16c0bdf: DashboardView danger-menu hover measured 4.38:1 (below AA) because --neg-soft was applied without its --neg-ink pair, now 8.31:1; StoreThemeCode `.tc-rail__btn:hover` out-specified `--active` so the active Explorer button lost its treatment on hover; DaVinciCopilot hid the history rail below 960 where it is the page's ONLY history surface (MpDaVinciBot is headerless) — the two panes now stack, so history stays reachable and the chat gets 375px instead of 115px. Also fixed one pre-existing defect of the same class: JourneyTemplateSetup's `.jts-intro` min-width used a kebab spelling of a camelCase token, so the declaration was dropped.
- 2026-09-05 — **SYSTEMIC FINDING, still open: the pass's 375px success criterion was measured at the wrong level.** Rows across this tracker record "0 overflow at 375px", but that measured `document.documentElement.scrollWidth`, which is always 375 because the card clips and scrolls internally. Measured at the table instead, polished pages still side-scroll inside their card at 375 and push the row kebab out of view: Coupons 276px, Collections 149px, ProductsList 142px, PriceLists 89px. Cause is the `minWidth: '200–280px'` literal every title column carries plus Vuetify's 16px cell padding against 311px of usable card width. Fixing it is a design-system decision (which columns may truncate, or halve cell padding below sm) affecting ~8 list views, so it wants its own pass rather than per-page patches. Re-verify with `table.scrollWidth === table.clientWidth`, not the document's.
- 2026-09-05 — **Re-verification closed the gap left by the usage limit.** The first audit run lost 67 of its skeptic agents mid-flight, which silently scored their findings "refuted" with zero votes. Re-running (all 135 agents, 0 errors) took the confirmed count from 3 to **10 upheld of 64 candidates**. The nine actionable ones are fixed in b6ad152 and c5faa55:
  - **Search silently stopped matching hidden columns** — the pass's own `hideBelow` tiering dropped fields out of Vuetify's search index, which only covers RENDERED columns. On SmsCampaigns and TransactionalSms the toolbar counted a match while the table said "No data available" below 960px; both pages already had a computed that searches every field, so the table's redundant `:search` is gone. TaxCategories had no such computed, so its search moved into one. **`:filter-keys` does NOT fix this** — tried first and verified failing in the browser.
  - **CommerceCloudLanding** — the hero chart's svg was absolutely positioned with `inset` + `width/height: auto`, so it resolved its height from the viewBox ratio, rendered 163px past its box and painted over the footer cells; it is back in flow. And the `!important` the pass removed from the primary CTA's `padding-inline` was load-bearing (the theme writes that property as an INLINE style, which no class selector can beat) — "Talk to Sales" had lost half its padding, 28px → 14px.
  - **ChatbotBuilder** — the ≤1024 rule was defeated by Vuetify's `overflow-hidden` / `flex-grow-1` utilities on the same elements, since those carry `!important`; only `flex-shrink` survived and the settings panel collapsed. Flex and overflow ownership moved into the scoped stylesheet.
  - **Field-group fills** — RelationalTables and Segments changed a near-white wash to `--surface-secondary` behind transparent outlined fields, taking their resting border to 2.92:1 against the enforced 3:1. Both now use a hairline instead; measured 3.45:1.
